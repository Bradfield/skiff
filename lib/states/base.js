...
class Base extends EventEmitter {
  ...
  _handleCommand (message, done) {
    ...
    this._node.command(
      ...
      (err, result) => {
        ...
        if (err) {
          this._node.network.reply(message.from, message.id, {
            replyTo: 'Command',
            term: currentTerm,
            error: {
              message: err.message,
  ...
  _perhapsGrantVote (message) {
    ...
    // - implement core logic of _perhapsGrantVote
    // - a node should grant a vote request if
    //   - it has not voted for anyone before
    //   - if it has then
    //     - the candidate must have a greater or equal term
    //       - if the term is the same then the candidate must be this nodes last vote
    //     - the candidate must have the same log or a newer log
  ...
  _appendEntriesReceived (message, done) {
    ...
    if (termIsAcceptable) {
      ...
      prevLogMatches =
        !params.prevLogIndex ||
        (!entry &&
          (log._lastLogIndex === params.prevLogIndex &&
            log._lastLogTerm === params.prevLogTerm)) ||
        (entry &&
          entry.t === params.prevLogTerm &&
          entry.i === params.prevLogIndex)

      debug('%s: previous log matches: %j', this.id, prevLogMatches)
      if (!prevLogMatches) {
        reason = `prev log term or index does not match: ${entry
        ...
        debug('%s: %s', this.id, reason)
        debug(
          '%s: last log index: %j, last log term: %j',
          this.id,
          log._lastLogIndex,
          log._lastLogTerm
        )
    ...
    function reply () {
      ...
      debug(
        'AppendEntries replied with success = %j to %s',
        success,
        message.from
      )
